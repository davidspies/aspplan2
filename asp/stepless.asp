...
% Helper function to recognize subsequent occurences of the same fluent/action.
...
% Any fluent above level 0 which holds must have exactly one causing action.
...
% For each precondition an action occurrence has, some occurrence of that fluent
% must permit it.
...
% For each subgoal condition, exactly one occurrence of that fluent permits it.
{permits(fluentOcc(F,M),subgoal(F)) : is(fluentOcc(F,M))}=1 :- subgoal(F).
% A fluent which permits a subgoal condition cannot be deleted.
:- deleted(FO); permits(FO,subgoal(_)).

% An occurrence of an action deletes one of a fluent if it permits it and that
% action has the fluent as a delete effect.
...
% An occurrence of a fluent doesn't hold if its previous occurrence doesn't hold.
:- holds(fluentOcc(F,M+1)); not holds(fluentOcc(F,M)); is(fluentOcc(F,M)); M > 0.
% An occurrence of an action doesn't hold if its previous occurrence doesn't hold.
...
% If a fluent permits an action, then the action happens after the start of the
% fluent
...
% For each vertex V and each cut C, V is on either the s-side or the t-side of
% V. Note this rule is disjunctive.
...
% If there's a directed edge from D to E, but D is on the t-side and E is on the
% s-side, this is not a cut (invalidating this counterexample to strong minimality).
...
% Action occurrence AO is not between cut1 and cut2 if it's on the s-side of
% cut1 or the t-side of cut2.
...
saturated(fluent(F)) :- holds(fluentOcc(F,M)) : is(fluentOcc(F,M)),M>0; fluent(F).
% An action is saturated if all occurrences of it happen.
saturated(action(A)) :- happens(actOcc(A,N)) : is(actOcc(A,N)); action(A).

% If an action happens in the suffix layer and all of its preconditions are
% subgoals, we designate it a "starting" action.
